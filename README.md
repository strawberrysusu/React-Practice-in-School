...
삼항연산자는 조건에 따라 서로 다른 내용을 보여줘야 할 때 사용하는 표현식이다.
...
React에서는 화면에 보여줄 내용을 조건에 따라 나누고 싶을 때 자주 사용한다.

```tsx
function UserStatus({ isLogin }: { isLogin: boolean }) {
  return (
    <div>
...
즉 삼항연산자는 두 가지 결과 중 하나를 선택해서 렌더링할 때 유용하다.

### JSX 중첩을 위한 줄바꿈과 소괄호

JSX를 한 줄로만 작성하는 것이 아니라, 여러 태그가 겹치거나 구조가 길어질 때는 줄바꿈을 해서 작성하는 경우가 많다.

이때 JSX 전체를 소괄호 `()`로 감싸면 읽기가 쉬워지고, `return` 뒤에 여러 줄로 JSX를 안정적으로 작성할 수 있다.
...
만약 JSX 구조가 중첩되어 길어질수록 줄바꿈과 들여쓰기를 함께 사용하는 것이 가독성에 도움이 된다.

또한 `return` 뒤에 바로 JSX를 여러 줄로 적을 때는 소괄호로 묶어 두는 방식이 일반적이다.
